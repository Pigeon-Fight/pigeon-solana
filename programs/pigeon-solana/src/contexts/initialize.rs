...
use crate::account::PigeonConfig;
...
pub struct Initialize<'info> {
    ...
    #[account(
        ...
        bump,
        space = 40 // 8 (discriminator) + 32 bytes pubkey
    )]
...
impl<'info> Initialize<'info> {
    pub fn initialize(&mut self, bumps: &InitializeBumps) -> Result<()> {
        // Set Admin Pubkey
        let pigeon_config_account = &mut self.pigeon_config;
        pigeon_config_account.admin_key = self.user.key();
